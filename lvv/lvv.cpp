...
{
    return db::read_int_db().value_or(utils::generate_n_random_ints(min_num_ints, INT_MIN, INT_MAX));
}
/** A set of random integers */
const unordered_set<int> INT_SET = fetch_int_set(db::NUM_INTS);

/**
 * @brief INTERNAL: the timed test function. The caller will be timing this function, so
 *        it should not do any blocking behavior
 */
void inline test_n_core_(IntegerSequence& seq, size_t num_vals, const vector<size_t>& removal_indices)
{
 ...
 * @param removal_indices a vector dictating the order in which to remove elements.
 ...
 */
chrono::nanoseconds test_n_(IntegerSequence& seq, size_t num_vals, const vector<size_t>& removal_indices, promise<chrono::nanoseconds> promise, size_t num_runs = 3)
{
 ...
 */
pair<chrono::nanoseconds, chrono::nanoseconds> test_n(size_t num_vals, size_t num_runs)
{
    ...
    // agh now we've duplicate dknowledge because num_vals == removal_indices.size()
    ...
    jthread vec_thread(test_n_, ref(v), num_vals, ref(removal_indices), move(vec_promise), num_runs);
    jthread list_thread(test_n_, ref(l), num_vals, ref(removal_indices), move(list_promise), num_runs);
...
void test_block(size_t start, size_t end, ostream& output)
{
    for (size_t i = start; i < end; i++) {
        ...
        output << i 
            << "," << vec_duration.count() 
            << "," << list_duration.count() 
            << "," << list_duration.count() - vec_duration.count() 
            << endl;
    }
}
 ...
 * @return size_t the number of tests to run (#DEFAULT_NUM_TESTS if no argument is found)
 */
