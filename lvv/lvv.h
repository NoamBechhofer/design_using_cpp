...
public:
    explicit ListAdaptor(std::list<int> const& l)
        : l(l)
    {
    }
    ListAdaptor() = default;
...
public:
    explicit VectorAdaptor(std::vector<int> const& v)
        : v(v)
    {
    }
...
extern std::pair<std::chrono::nanoseconds, std::chrono::nanoseconds> test_n(size_t num_vals, size_t num_runs = DEFAULT_RUNS_PER_TEST);
